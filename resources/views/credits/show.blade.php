...
<div class="wrapper">
    ...
    <div class="mt-4">
    <a href="{{ route('credits.generate-pdf', ['credit' => $credit->id]) }}" class="btn btn-info">Pdf</a>
        <a href="{{ route('credits.edit', $credit->id) }}" class="btn btn-info">Edit</a>
        <a href="{{ route('avis.history', ['id_credit' => $credit->id]) }}" class="btn btn-info">the history of avis</a>

        <form action="{{ route('avis.create') }}" method="get">
            @csrf
            <input type="hidden" name="id_credit" value="{{ $credit->id }}">
            <button type="submit" class="btn btn-info">Create Avi</button>
        </form>
